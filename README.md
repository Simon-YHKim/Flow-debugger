...
상단 **AI 하네스** 버튼을 누르면, 앱의 AI가 어떻게 엮여 도는지(입력 → 안전검사 → callGemini 게이트웨이 → 티어 → 엣지 프록시·스펜드캡 → 모델 → 출력검사 → 감사로그)와 각 단계의 역할을 한눈에 봅니다:

![AI 하네스 — 게이트웨이·안전검사·티어·프록시·모델·감사 배선](docs/harness.png)
