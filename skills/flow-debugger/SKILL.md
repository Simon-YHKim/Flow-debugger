...
- **노드 추가**: 우측 '노드 추가' 탭에서 "이런 화면·기능이 있어야 해요" 노드를 얹고 목록에서 삭제(프롬프트에 '만들어줘'로 자동 기록)
- **시스템 스펙**: 상단 **📋 시스템 스펙** 버튼 → 팝업으로 스택·화면(엔드포인트/명령)/동작 수·서버·데이터 작업 인벤토리·AI 목록·위험 프로필을 자동 집계(로드된 데이터에서 계산, 대상 종류 무관)
- **미니맵**: 좌하단 미니맵을 클릭·**드래그**해서 큰 흐름도에서 원하는 위치로 빠르게 이동
- **대상 모드**: `<graph>.mode.txt` = ui(화면·기본) / backend(엔드포인트) / cli(명령) — 화면 없는 API 서버·CLI 도구도 같은 스키마로 맵핑(제목·통계·라벨·스펙 어휘 자동 전환). 매핑 가이드는 references/scan-prompts.md "대상 모드"
