...
## 0단계 PRESCAN — 리더가 스스로 알 수 없는 사실 (필수)

실측: 이 스킬을 86화면 앱에 돌렸더니 **27화면만 정확**했다. 275건의 오류 중 셋은
"모델이 코드를 잘못 읽어서"가 아니라 **"눈앞의 파일만 봐서는 답할 수 없는 질문을 시켜서"** 생겼다.
그래서 스캔 전에 스크립트로 먼저 답을 구해 프롬프트에 **붙여 넣는다**.

```bash
node scripts/prescan.js <appRoot> --out Output/prescan.json
```

이게 답하는 세 가지 — 그리고 안 하면 무슨 일이 생기는지:

| 질문 | 안 물으면 | 실측 피해 |
|---|---|---|
| **프로덕션은 둘 중 어느 컴포넌트를 그리는가?** | 라우트 파일이 `if (isXxxUI()) return <NewScreen/>` 로 위임하는데, 리더가 그 위의 legacy 본문을 화면이라고 적는다 | 화면 10개가 **아예 다른 화면** |
| **사용자가 이 코드에 닿는가?** | `DevOnlyRoute` 로 배포판에선 열리지도 않는 화면의 버튼을 "가짜 저장 — 데이터 손실"이라고 신고한다. 코드는 맞게 읽었고 결론이 틀렸다 | **허위 버그 4건** (가장 치명적) |
| **이 헬퍼가 속으로 뭘 하는가?** | 화면엔 `createRecord(...)` 한 줄뿐인데 그 안에서 임베딩 모델을 부른다. 화면만 읽으면 절대 안 보인다 | 서버 호출 **66건**·AI **7건** 누락 |

prescan 출력의 세 블록을 **SCAN 프롬프트 맨 위에 그대로 붙인다**:
1. `프로덕션 렌더 경로` → "사용자가 보는 화면은 이 함수가 고르는 쪽이다"
2. `겉보기와 다른 헬퍼` 표 → 화면에 그 함수 호출이 보이면 표의 apis/ai 를 그대로 적게 한다
3. `게이트가 걸린 화면` → **이 화면의 동작에는 `bug` 위험을 붙이기 전에 "프로덕션에서 열리는가"를 먼저 답한다**
...
>      **⚠ bug 를 붙이기 전에 반드시: "이 코드에 프로덕션 사용자가 닿는가?"**
>      dev 전용 라우트·기능 플래그·등급 게이트 뒤라면 bug 를 붙이지 말고 `gate` 만 붙이고,
>      detail 에 "배포판에선 열리지 않음"이라고 적는다. (게이트를 안 물어서 "저장 버튼이 가짜다,
>      데이터가 사라진다"는 확신에 찬 허위 신고가 나온 적이 있다 — 그 화면은 사용자가 열 수조차 없었다.)
...
SCAN · ENRICH · ANNOTATE 를 **한 번에** 시키지 말 것. 토큰을 아끼려고 리더 하나에게
"화면 9개를 읽으면서 동시에 동작을 뽑고, 한국어로 옮기고, 위험도 판정해" 를 시킨 적이 있다.
결과: 동작 90개 누락, 서버 호출 66개 누락, 앵커 43개 오류. **아낀 토큰은 검증·수리에 10배로 지불했다.**
화면이 많으면 합치지 말고 **그룹을 쪼개서 병렬로** 돌린다.
